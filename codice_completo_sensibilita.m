...
phi_ice_cl_vect = [0.1 0.3];
phi_ice_cr_vect = [0.1 0.2 0.3];
phi_ice_de_vect = [0.1 0.3];
Hp_vect = [0.3 0.4]; % fattore di ibridizzazione
t_c_vect = [0.1 0.2 0.3];
lambda_vect = [0.4 0.5 0.6];
AR_vect = [9 10 11];
...
indice_config = 1;
tot_config = length(AR_vect)*length(lambda_vect)*length(t_c_vect)*length(W_S_vect)*length(Hp_vect)*length(phi_ice_cl_vect)*length(phi_ice_cr_vect)*length(phi_ice_de_vect);
% ciclo di dimensionamento
for i_AR = 1:length(AR_vect)
    for i_lambda = 1:length(lambda_vect)
        for i_tc = 1:length(t_c_vect)
                    ...
                    for i_phi_ice_cl = 1:length(phi_ice_cl_vect)
                        for i_phi_ice_cr = 1:length(phi_ice_cr_vect)
                            for i_phi_ice_de = 1:length(phi_ice_de_vect)
                                ...
                                AR_des = AR_vect(i_AR);
                                lambda_des = lambda_vect(i_lambda);
                                t_c_des = t_c_vect(i_tc);
                                ...
                                if all(phi_em <= 1) && all(phi_em >= 0)
                                    ...
                                    AR_memo(indice_contatore) = AR_des;
                                    t_c_memo(indice_contatore) = t_c_des;
                                    lambda_memo(indice_contatore) = lambda_des;
...
data = struct( ...
    ...
    'AR', AR_memo(1:indice_contatore), ...
    'lambda', lambda_memo(1:indice_contatore), ...
    't_c', t_c_memo(1:indice_contatore), ...
...
writetable(struct2table(data), 'dati_convergenza_sensibilita.csv');
