...
n_step = 2*ceil(taxi_time/dt) + ceil(takeoff_time/dt) + ceil(h_cruise/(ROC*dt)) + ceil(range_cruise*1e3/(V_cruise*dt)) + ceil(abs(h_cruise/(ROD*dt))) + ...
    ceil(h_cruise_diversion/(ROC_diversion*dt)) + ceil(range_cruise_diversion*1e3/(V_cruise_diversion*dt)) + ceil(abs(h_cruise_diversion/(ROD_diversion*dt)));
...
while z > 0
    ...
    % consumo batteria
    E_batt_dot(i_descent_diversion) = 0; % diversione in full thermal
    E_batt(i_descent_diversion + 1) = E_batt(i_descent_diversion) + E_batt_dot(i_descent_diversion) * dt * sec2hr; % [W*h]
    phi_em(i_descent_diversion) = E_batt_dot(i_descent_diversion) / P_em;
    % consumo carburante
